...
object JokeBox {
  ...
  final case class ScheduleMusic(tracks: List[String], replyTo: ActorRef)
      extends MusicBox
  final case class DownloadMusic(
      track: String,
      replyTo: akka.actor.typed.ActorRef[DownloadInfo]
  ) extends MusicBox
  ...
  final case class SubscribeMusic(
      replyTo: akka.actor.typed.ActorRef[Subscribed]
  ) extends MusicBox
